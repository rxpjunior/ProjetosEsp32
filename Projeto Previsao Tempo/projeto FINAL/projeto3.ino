...
retornoConsulta = httpGETRequest(serverName);
      ...
      //Montando a String que será enviada via Websocket
      envioWebsocket = envioWebsocket+probChuva;
      envioWebsocket = envioWebsocket+","; 
      envioWebsocket = envioWebsocket+tempMin;
      envioWebsocket = envioWebsocket+","; 
      envioWebsocket = envioWebsocket+tempMax;
 ...
 //Mantém o servidor WebSocket em execução, verificando por novas conexões e mensagens
  ...
  if ((millis() - ultimoDisparo) > tempoDecorrido) {
    ...
    if(WiFi.status()== WL_CONNECTED){
      buscaDadosApi();
      webSocket.broadcastTXT(envioWebsocket);
      envioWebsocket = "";         
    }
    else {
      exibeMsgConfWifi();
